document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('generateBtn').onclick = async () => {
        ...
        try {
            // Live Render API connection for generation
            const response = await fetch('https://openclaw-social-api.onrender.com/generate-drafts', {
                method: 'POST',
        ...
        drafts.forEach((draft) => {
            ...
            btn.onclick = async () => {
                ...
                chrome.storage.local.get(['handle', 'app_password'], async (creds) => {
                    ...
                    try {
                        // Live Render API connection for posting
                        const res = await fetch('https://openclaw-social-api.onrender.com/post-to-bluesky', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                handle: creds.handle,
